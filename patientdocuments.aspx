            ...
            <div class="row">
                <asp:GridView ID="gvDocument" BorderStyle="None" CssClass="table table-bordered" AutoGenerateColumns="false" Width="100%" runat="server">
                    <Columns>
                        ...
                        <asp:TemplateField>
                            <ItemTemplate>
                                <asp:Button runat="server" ID="btnDelete" CommandArgument='<%# Eval("Document_ID") %>' CssClass="btn btn-danger btn_size" OnClientClick="return confirm('Are you sure you want to delete this document ?')" Text="Delete" CausesValidation="false" OnClick="btnDelete_Click" />
                                  <asp:Button runat="server" ID="Button1" CommandArgument='<%# Eval("path")+"#"+Eval("DocName") %>' CssClass="btn btn-warning btn_size" Text="Download" CausesValidation="false" OnClick="btnDownload_Click" />
                                <%--  <asp:Button runat="server" ID="btnPreview" CommandArgument='<%# Eval("Document_ID") %>' CssClass="btn btn-info btn_size" Text="Preview" CausesValidation="false" OnClick="btnPreview_Click" />--%>
                                <a href='<%# "https://docs.google.com/viewer?url=https://www.paintrax.com/AKS"+Eval("path").ToString().Replace("~","")+"&embedded=true" %>' target="_new" class="btn btn-info btn_size">Preview </a>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
                </asp:GridView>
