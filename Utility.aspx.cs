...
public partial class Utility : System.Web.UI.Page
{
    ...
    protected void btnupload_mul_Click(object sender, EventArgs e)
    {
     ...
     protected void btnupload_zip_Click(object sender, EventArgs e)
    {
        DBHelperClass dBHelperClass = new DBHelperClass();
        DataSet dataSet = new DataSet();
        string fname = "", lname = "";

        string pathToZip = Server.MapPath("~/PatientDocument/Zip/");
        string destination = Server.MapPath("~/PatientDocument/Destination/");

        if (fupZip.HasFile)
        {
            pathToZip = pathToZip + fupZip.FileName;
            fupZip.SaveAs(pathToZip);



            string zipPath = pathToZip;
            string extractPath = destination;
            string getFilePath = destination + Path.GetFileNameWithoutExtension(fupZip.FileName);
            ...
            foreach (var f in files)
            {
                ...
                try
                {
                    ...
                    if (dataSet != null && dataSet.Tables[0].Rows.Count > 0)
                    {
                        ...
                        f.CopyTo(System.IO.Path.Combine(Server.MapPath(upload_folder_path), fileName));
