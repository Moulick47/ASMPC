...
public partial class Page1 : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Request["id"] == null )
        {
            _strResult = this.checkDuplicatePatient(txt_fname.Text, txt_mname.Text, txt_lname.Text, txt_DOB.Text);
        }
        ...
        param[42] = new SqlParameter("@Vaccinated", ddlVaccinated.SelectedItem.Value);
        string patientIE_ID = "";
        ...
        if (!string.IsNullOrEmpty(patientIE_ID))
        {
            ...
            param[43] = new SqlParameter("@PatientIE_ID", Session["PatientIE_ID"]);
        }
        ...
        {
            ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), Guid.NewGuid().ToString(), "checkDuplicate('" + txt_fname.Text + "','" + txt_mname.Text + "','" + txt_lname.Text + "','" + _strResult + "')", true);
        }

    }

    public string checkDuplicatePatient(string fname, string mname, string lname, string dob)
    {
        string _strDOA = "";

        string _strQuery = "select DOE from View_PatientIE where FirstName='" + fname + "' and LastName='" + lname + "' and MiddleName='" + mname + "' and CAST(DOB as date)=CAST('" + dob + "' as date) order by [PatientIE_ID] desc";

        DataSet _objDataSet = db.selectData(_strQuery);

        if (_objDataSet != null && _objDataSet.Tables[0].Rows.Count > 0)
        {
            _strDOA = _objDataSet.Tables[0].Rows[0][0].ToString();
        }
